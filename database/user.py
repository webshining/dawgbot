...
class User(Base):
    ...
    @classmethod
    @execute
    async def get(cls: type[T], id: int = None, session: Surreal = None) -> T | list[T] | None:
    ...
    async def get_by_notifications(
        cls: type[T], server_id: int, channel_id: int, session: Surreal = None
    ) -> list[int]:
    ...
    @classmethod
    @execute
    async def add_server(cls, id: int, server_id: int, session: Surreal = None):
    ...
    @classmethod
    @execute
    async def remove_server(cls, id: int, server_id: int, session: Surreal = None):
        await session.query(f"DELETE user:{id}->user_server WHERE out = server:{server_id}")
        return await cls.get(id=id, session=session)
